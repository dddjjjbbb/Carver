...
from src.common.app_io.reader.reader import get_books_already_scraped, get_books_to_scrape, read_file
...
def main():
    ...
    parser.add_argument("-f", "--format", type=str, default="json", choices=["json", "csv"])
        ...
        for book_id in books_ids_to_scrape:
            ...
            try:
                ...
                if book_model.get("authorFullName") is None:
                    logging.warning(f"Skipping: {book_id} Author is None")
                else:
                    write_to_json(book_model, path)

            except Exception as e:
                logging.error(f"Skipping: {book_id}. See: {e}")
                raise e
