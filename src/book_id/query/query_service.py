...
class QueryService:
    ...
    def build_query_model(self) -> Union[QueryModel, IndexError]:
        try:
            ...
            raise IndexError(
                f"Please check if the delimiter is present and as specified. See: {e}"
            )

        search_query = f"{book_title} - {author_name}"

        if QueryService._is_subtitle_in_book_title(book_title):
            book_title_minus_subtitle = QueryService._remove_subtitle_from_book_title(
                book_title
            )
        else:
        ...
        return QueryModel(
            ...
            book_title_and_author_name_search_url=QueryService._build_search_url_from_query(
                search_query
            ),
            book_title_search_url=QueryService._build_search_url_from_query(book_title),
            book_title_minus_subtitle_search_url=QueryService._build_search_url_from_query(
