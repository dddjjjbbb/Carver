...
from get_ids.result.result_parser import (
    _clean_author_name,
    _get_author_name,
    _get_book_href,
    _get_book_title,
...
class TestResultParser:
    ...
    def test_ut_clean_author_name_should_remove_goodreads_author_parenthesis_where_present(
        ...
        assert _clean_author_name(self.author_name_with_goodreads_author_parenthesis) == "Banana Yoshimoto"

    def test_ut_clean_author_name_should_return_goodreads_author_where_goodreads_author_parenthesis_is_not_present(
        self,
    ):
        assert _clean_author_name(self.author_name_without_goodreads_author_parenthesis) == "Banana Yoshimoto"
    ...
    def test_ut_get_book_href_should_return_book_href_where_book_href_is_present(self):
        result = self.results[1]
        assert _get_book_href(result) == "/book/show/50144.Kitchen?from_search=true&from_srp=true&qid=JBRJrWZAoO&rank=2"
