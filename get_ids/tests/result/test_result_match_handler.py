...
class TestResultMatchHandler:
    ...
    def test_ut_is_results_equal_to_one_should_return_true_where_result_is_equal_to_one(
        self,
    ):
        assert _is_results_equal_to_one(self.result_model) is True

    def test_ut_is_results_equal_to_one_should_return_false_where_result_is_not_equal_to_one(
        self,
    ):
        assert _is_results_equal_to_one(self.empty_list) is False

    def test_ut_is_input_similar_returns_true_where_expecting_an_exact_match(self):
        assert _is_input_similar("Kitchen", "Kitchen", 1.0) is True

    def test_ut_is_input_similar_returns_false_where_expecting_an_exact_match(self):
    ...
    def test_it_get_match_returns_none_where_result_models_are_empty(
        self,
    ):
